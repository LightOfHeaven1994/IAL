...
*/	
	if(q != NULL){
		for(int i = 0; i < QUEUE_SIZE; i++){
			q->arr[i] = '*';					/*Vsechny hodnoty oznacime '*' */
		}
	}
	else{
		queueError(QERR_INIT);					/*Pokud q == NULL zavolame funkce s hybou*/
	}

	q->f_index = 0;								/*Nastavime index zacatku fronty na 0*/
	q->b_index = 0;								/*Nastavime index prvniho volneho mista na 0*/
...
*/
	return ((index + 1) % QUEUE_SIZE);		/*pokud prestoupi konecny index tak pujde od zacatku*/
...
*/
	return (q->f_index == q->b_index)? 1 : 0;	/*Overeni je li fronta plna*/
...
*/
	return (q->f_index == nextIndex(q->b_index))? 1 : 0;	/*Overeni je li fronta plna*/
}
...
*/
	if(queueEmpty(q)){				/*Volani funkce pro overeni je li fronta prazdna*/
		queueError(QERR_FRONT);
	}
	else{
		*c = q->arr[q->f_index]; 	/*Ulozi znak ze zacatku fronty na adresu kam ukazuje ukazatel */
	}
}
...
*/
	if(queueEmpty(q)){							/*Volani funkce pro overeni je li fronta prazdna*/
		...
		q->f_index = nextIndex(q->f_index);		/*Odstranime znak ze zacatku fronty*/
	}
...
*/	
	if(queueEmpty(q)){				/*Volani funkce pro overeni je li fronta prazdna*/
		...
		queueFront(q, c);			/*S pomoci funkce Front ulozime znak ze zacatku fronty*/
		queueRemove(q);				/*S pomoci funkce odstranime znak ze zacatku fronty*/
	}
...
*/
	if(queueFull(q)){								/*Volani funkce pro overeni je li fronta prazdna*/
		queueError(QERR_UP);
	}
	else{
		q->arr[q->b_index] = c;						/*Ulozime zmak c na volne misto kam ukazuje b_index*/
		q->b_index = nextIndex(q->b_index);			/*Posuneme b_index na 1 pozici dopredu*/
	}
}
